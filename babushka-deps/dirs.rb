meta 'dirs' do
	...
	template {
		...
		def set_permission(dir); log_shell "Setting permission #{mask} on #{dir}", "chmod #{mask} #{dir}", :sudo => true ; end
		def set_owner(dir); log_shell "Setting owner #{user}:#{group} on #{dir}", "chown #{uid}:#{gid} #{dir}", :sudo => true; end
