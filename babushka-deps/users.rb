...
meta 'userkey' do
	template {
		...
		def permitted?; File.lstat(ssh_path).mode.to_s(8) == "40700"; end
		def ownership?; stat(ssh_path).uid == uid && stat(ssh_path).gid == gid; end
		def fix_permissions; shell "chmod 700 -R #{ssh_path}"; end
		def fix_ownership; shell "chown #{basename}:#{basename} -R #{ssh_path}"; end
		def contains_key?; shell "grep '#{pub_key}' #{authorized_keys_path}"; end
		def create_ssh_path; shell "mkdir -p -m 700 #{ssh_path}"; end
